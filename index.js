...
app.get('/api/intern-uploads/:email', async (req, res) => {
    ...
    try {
        ...
        // Fetch uploaded files for the intern
        const filesResult = await pool.query(
            'SELECT file_name, original_name, uploaded_at FROM uploaded_files WHERE intern_id = $1',
            [internId]
        );

        if (filesResult.rows.length === 0) {
            return res.status(404).json({ error: 'No files uploaded by this intern' });
        }
...
// Update internship request status and associate the intern with the company
app.put('/api/company-internship-requests/:id/accept', async (req, res) => {
    ...
    try {
        ...
        const requestResult = await pool.query(
            `SELECT company_id, intern_id FROM companyinternshiprequests WHERE id = $1`,
            [requestId]
        );
        ...
        const { company_id, intern_id } = requestResult.rows[0];
        ...
        // Add the intern to the company's intern list (if applicable)
        // Assuming a relationship between companies and interns in another table
        await pool.query(
            `INSERT INTO company_interns (company_id, intern_id, joined_at)
             VALUES ($1, $2, CURRENT_TIMESTAMP)
             ON CONFLICT (company_id, intern_id) DO NOTHING`, // Avoid duplicates
            [company_id, intern_id]
        );

        res.json({ message: 'Internship request accepted, and intern added to the company.' });
    } catch (error) {
...
app.delete('/api/company-dismiss-intern/:internId', async (req, res) => {
    ...
    try {
        // Remove the intern from the company_interns table
        ...
        res.json({ success: true, message: 'Intern dismissed successfully.' });
    } catch (error) {
        console.error('Error dismissing intern:', error);
        res.status(500).json({ error: 'Failed to dismiss intern.' });
    }
});
